...
target = df['High'][-1]                                 # choose end of selection
df = df.reset_index()
print(df)
...
selection_df_high = selection_df['High'].iloc[-1]       # get last row of selection for reversal but first row for copy
diff = selection_df_high - target                      # get the diff
print(diff)

# use for reversal
selection_df = selection_df.iloc[::-1]

# calculate y axis adjustment
...
# don't do this for putting it at the end
